...
export class HealthStateSyncer {
  ...
  async maybeSyncHealthScoreWithDb(localHealthScoreDiff: number, localHealthScore: number): Promise<HealthSyncResult> {
    ...
    let readResult: HealthReadResult | null = null
    try {
  ...
  private async readHealthScoreFromDb(): Promise<HealthReadResult | null> {
    const getParams = {
